...
exports.authenticate = async (email, password) => { 
    ...
    if(isCorrectPW) {
        ...
        user.updateToken(id_token,refresh_token, access_token, id_user)
        return [{isCorrectPW, id_token, access_token, id_user}, refresh_token]
...
exports.renewAccessToken = async (refresh_token, id_token) => { 
    try {
        const {id, email, iat, exp} = jwt.verify(refresh_token, process.env.SECRET_KEY); 
        
        id_token = token.generateIdToken({id: id, email: email})
        access_token = token.generateAccessToken({id: id, email: email})
        user.updateToken(id_token,refresh_token, access_token, id)
